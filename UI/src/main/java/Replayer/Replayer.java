...
public class Replayer extends JFrame {
    ...
    private void createActionListeners(JButton findGame, JButton doNextMove, JButton doPreviousMove) {
        findGame.addActionListener(event -> {
                    ...
                    if (boardFromDB.isEmpty()) {
                        ...
                        int blackPlayerIdFromDB = database.getPlayerIdFromGame(gameIdTextField.getText(), 'b');
                        int whitePlayerIdFromDB = database.getPlayerIdFromGame(gameIdTextField.getText(), 'w');
    ...
    private void updateBoard(Boards boardFromDB, int numberOfTurnForTurn) {
        Board board = BoardParser.parse(boardFromDB.board, 'b', 'w', ' ');
