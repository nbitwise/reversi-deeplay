...
public class Commands {
     ...
     */
    public static void addAllCommands(@NotNull final List<Command> commands) {
        addRegistrationCommand(commands);
        addAuthorizationCommand(commands);
        addCreateRoomCommand(commands);
        addConnectToRoomCommand(commands);
     ...
     * @param commands - список команд
     */
    private static void addRegistrationCommand(@NotNull final List<Command> commands) {
     ...
     * @param commands - список команд
     */
    private static void addAuthorizationCommand(@NotNull final List<Command> commands) {
     ...
     * @param commands - список команд
     */
    private static void addCreateRoomCommand(@NotNull final List<Command> commands) {
        commands.add(Command.newCommand("CREATEROOM", (jsonRequest, uuid) -> {
            if (onlineUsers.containsKey(uuid)) {
                ...
                room.blackPlayerId = 1;//1204371203701273!!!!!!!!!!!!!
                room.roomId = random.nextInt(Integer.MAX_VALUE);

                roomList.add(room);
                System.out.println(room.roomId);
                return new CreateRoomResponse("success", "Room was successfully registered", room.roomId);
            }

            return new CreateRoomResponse(FAIL_STATUS, YOU_ARE_NOT_LOGGED_IN, null);
        }));
    }
     ...
     * @param commands - список команд
     */
    private static void addConnectToRoomCommand(@NotNull final List<Command> commands) {
        commands.add(Command.newCommand("CONNECTTOROOM", (jsonRequest, uuid) -> {
            if (onlineUsers.containsKey(uuid)) {
                ...
                if (room != null && room.hasNoPlayers()) {
                    ...
                    room.whitePlayerId = 2;//@@@@@@&$9193264916!!!!!!!!!!!!
        ...
        commands.add(Command.newCommand("SURRENDER", (jsonRequest, uuid) -> {
            if (onlineUsers.containsKey(uuid)) {
                ...
                if (room != null) {
                    ...
                    opponentClientProcessor.sendReply(new SurrenderResponse("Your opponent has surrendered \n" + stringReply));
                    return new SurrenderResponse("You surrendered %n" + stringReply);
                } else {
                    return new SurrenderResponse(YOU_ARE_NOT_IN_ROOM);
                }
            }
            return new GameoverResponse(FAIL_STATUS, YOU_ARE_NOT_LOGGED_IN);
        }));
