    ...
    private static void addRegistrationCommand(@NotNull final Database database, @NotNull final List<Command> commands) {
        commands.add(Command.newCommand("REGISTRATION", (jsonRequest, uuid) -> {
            ...
            if (nickname == null || !nickname.matches("^[\\w]+$")) {
                return new ResponseRegistration(FAIL_STATUS, "incorrect username");
            }
            if (database.addPlayerOnDatabase(nickname)) {
                return new ResponseRegistration(SUCCESS_STATUS, "You was successfully registered");
            } else {
                return new ResponseRegistration(FAIL_STATUS, "User with this nickname already registered");
            }
        }));
    }
        ...
        commands.add(Command.newCommand("AUTHORIZATION", (jsonRequest, uuid) -> {
            ...
            if (database.checkRegistration(nickname)) {
                if (onlineUsers.containsValue(nickname)) {
                    return new ResponseAutorization(FAIL_STATUS, "user with this nickname already online");
                } else {
                    onlineUsers.put(uuid, nickname);
                    return new ResponseAutorization(SUCCESS_STATUS, "You have successfully logged in");
                }
            } else {
                return new ResponseAutorization(FAIL_STATUS, "You are not registered");
            }
        }));
    }
